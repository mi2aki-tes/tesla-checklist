...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        // 旧データ（photo: string | null）を新形式（photos: string[]）に移行
        if (parsed.itemsData) {
          Object.keys(parsed.itemsData).forEach((key) => {
            const item = parsed.itemsData[key];
            if (item.photo !== undefined && item.photos === undefined) {
  ...
  const handleUpdateItem = (id: string, partialData: Partial<ItemData>) => {
    setState((prev) => {
      const existing = prev.itemsData[id] || { status: null, memo: "", photos: [] };
      return {
        ...prev,
        itemsData: {
          ...prev.itemsData,
  ...
  // 写真があるアイテムとその写真を全て列挙（複数枚対応）
  type PhotoEntry = { itemTitle: string; itemCategory: string; memo: string; photo: string };
  const photoEntries: PhotoEntry[] = [];
  currentItems.forEach((item) => {
    const data = state.itemsData[item.id];
  ...
  // 4枚ずつページ分割（2列×2行）
  const photoChunks: PhotoEntry[][] = [];
  for (let i = 0; i < photoEntries.length; i += 4) {
    photoChunks.push(photoEntries.slice(i, i + 4));
  }
          ...
          <div key={category} className="mb-8">
            ...
            <div className="flex flex-col gap-2">
              {currentItems
                ...
                .map((item) => {
                  const data = state.itemsData[item.id] || { status: null, memo: "", photos: [] };
                  return (
                    <ChecklistItemComponent
                      ...
                      onPhotosChange={(photos) => handleUpdateItem(item.id, { photos })}
                    />
                  );
                })}
            </div>
      ...
      <footer className="max-w-2xl mx-auto px-4 pb-12 text-center">
        <div className="inline-flex flex-col items-center justify-center p-6 bg-white border border-gray-200 rounded-2xl shadow-sm w-full">
          ...
          <div className="flex justify-center items-center gap-6">
            <a href="https://www.youtube.com/@mi2aki_tesla" target="_blank" rel="noopener noreferrer" className="text-sm font-semibold text-gray-500 hover:text-red-600 transition-colors underline underline-offset-4">
              YouTube
            </a>
            <a href="https://x.com/mi2aki_tesla" target="_blank" rel="noopener noreferrer" className="text-sm font-semibold text-gray-500 hover:text-black transition-colors underline underline-offset-4">
              X (Twitter)
            </a>
          </div>
        </div>
      ...
      <div className="absolute top-0 left-0 -z-50 opacity-0 pointer-events-none">
        ...
        <div id="print-template" className="w-[800px] bg-white p-10 text-black">
          <h1 className="text-2xl font-bold border-b-2 border-black pb-2 mb-6">
            Tesla納車時チェックリスト（{state.selectedModel}）
          </h1>

          <table className="w-full text-left text-sm border-collapse">
            <thead>
              <tr className="bg-gray-800 text-white border-b-2 border-gray-800">
                <th className="py-2.5 px-4 font-bold w-20">結果</th>
                <th className="py-2.5 px-4 font-bold w-48">カテゴリ / 項目</th>
                <th className="py-2.5 px-4 font-bold">メモ</th>
              </tr>
            </thead>
            <tbody>
              {currentItems.map((item) => {
                ...
                return (
                  <tr key={item.id} className="border-b border-gray-200 even:bg-gray-50/80">
                    <td className={`py-2 px-4 align-top ${statusColor}`}>{statusText}</td>
                    ...
                    <td className="py-2 px-4 align-top">
                      ...
                      {photoCount > 0 && (
                        <span className="block mt-1 text-blue-600 font-semibold text-[10px]">
                          [写真{photoCount}枚 - 次ページ参照]
                        </span>
                      )}
                    </td>
                  </tr>
        ...
        {/* Pages 2+: Photo Grids（1ページ4枚、各写真に「何枚目か」を表示） */}
        {photoChunks.map((chunk, pageIndex) => (
          <div
            key={`photo-page-${pageIndex}`}
            className="print-photo-page w-[800px] bg-white p-10 text-black flex flex-col"
            style={{ minHeight: "1130px" }}
          >
            <h2 className="text-xl font-bold border-b-2 border-gray-300 pb-2 mb-6">
              添付写真 ({pageIndex + 1}/{photoChunks.length})
            </h2>
            <div className="grid grid-cols-2 gap-6 content-start">
              {chunk.map((entry, entryIndex) => (
                <div
                  key={`photo-${pageIndex}-${entryIndex}`}
                  className="flex flex-col gap-2 p-4 border border-gray-200 rounded-lg bg-gray-50"
                >
                  <div className="font-bold text-sm text-gray-800 leading-snug">{entry.itemTitle}</div>
                  <div className="text-[10px] text-gray-500">{entry.itemCategory}</div>
                  {/* メモが切れないよう whitespace-pre-wrap で折り返し */}
                  {entry.memo && (
                  ...
                  <img
                    src={entry.photo}
                    alt={entry.itemTitle}
                    className="w-full h-52 object-cover rounded shadow-sm border border-gray-300 bg-white mt-1"
                  />
                </div>
              ))}
            </div>
